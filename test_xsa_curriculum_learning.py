...
def main():
    ...
    if config_proj["check_point"] != None:
        ...
        print("test_model:{}".format(config_proj["check_point"]))
        model.to(device)
        ...
        # need to change
        test_sets = config_proj["Input"]["test_sets_10s_BCDFG"].split()
        for test in test_sets:
            # test = "data/lre17_eval_3s_5_snrs"
            print("## Test set: ", test)
            exp_dir = log_dir  + "/" +test
            if not os.path.exists(exp_dir):
            	os.mkdir(exp_dir)
            test_txt = config_proj["Input"]["userroot"] + "data-16k/" + test + "/wav2vec_" + config_proj["wav2vec_info"]["model_name"] + ".txt"
            test_feats = config_proj["Input"]["userroot"] + "data-16k/" +test +  "/wav2vec_" + config_proj["wav2vec_info"]["model_name"] + "_" + str(feat_layer) + "_layer/feats.scp"

            if test_txt is not None:
                validation(test_txt, test_feats, model, model_name, device, kaldi=kaldi_root, log_dir=exp_dir,
